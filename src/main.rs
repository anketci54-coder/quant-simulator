...
fn save_active_positions_to_db(conn: &Mutex<Connection>, positions: &[ActivePosition]) -> Result<(), String> {
    let c = conn.lock().map_err(|e| e.to_string())?;
...
fn load_active_positions_from_db(conn: &Mutex<Connection>) -> Vec<ActivePosition> {
                    ...
                    if let Some(t) = tickers.iter().find(|x| x.symbol == pos.symbol) {
                        if let Ok(curr_price) = t.last_price.parse::<f64>() {
                            ...
                            if pos.peak_pnl_percent >= 5.0 {
                                if pos.side == "LONG" {
                                    let trailed_sl = pos.highest_price * 0.99;
                                    if trailed_sl > pos.stop_loss { pos.stop_loss = trailed_sl; }
                                } else {
                                    let trailed_sl = pos.highest_price * 1.01;
                                    if trailed_sl < pos.stop_loss { pos.stop_loss = trailed_sl; }
                                }
                            }
